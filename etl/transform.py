...
def build_user_product_features(opp: pd.DataFrame, prior_orders: pd.DataFrame, user_feat: pd.DataFrame) -> pd.DataFrame:
    ...
    opp_u = prior_opp.merge(
        prior_orders[["order_id", "user_id", "order_number"]],
        on="order_id",
        how="left"
    )
    
    up_cnt = opp_u.groupby(["user_id", "product_id"], as_index=False).agg(up_cnt=("order_id", "count"))
    
    up_last_order = opp_u.groupby(["user_id", "product_id"], as_index=False).agg(up_last_order=("order_number", "max"))
    ...
    up_feat = up_feat.drop(columns=["user_order_cnt"])
    
    up_feat = up_feat.merge(up_last_order, on=["user_id", "product_id"], how="left")
    return up_feat

def assemble_training_table(opt_new, user_feat, product_feat, up_feat) -> pd.DataFrame:
    ...
    fill_cols = ["up_cnt", "up_last_order", "up_ratio", "product_cnt", "product_reorder_rate", "product_unique_user"]
    for c in fill_cols:
        if c in df.columns:
            df[c] = df[c].fillna(0)
